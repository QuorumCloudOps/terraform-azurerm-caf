resource "azurerm_monitor_diagnostic_setting" "diagnostics" {
  ...
  log_analytics_workspace_id     = each.value.destination_type == "log_analytics" ? try(try(var.diagnostics.diagnostics_destinations.log_analytics[each.value.destination_key].log_analytics_resource_id, var.diagnostics.log_analytics[var.diagnostics.diagnostics_destinations.log_analytics[each.value.destination_key].log_analytics_key].id),local.combined_objects_log_analytics[try(var.diagnostics.diagnostics_destinations.log_analytics[each.value.destination_key].lz_key, local.client_config.landingzone_key)][try(var.diagnostics.diagnostics_destinations.log_analytics[each.value.destination_key].log_analytics_key, null)].id) : null
